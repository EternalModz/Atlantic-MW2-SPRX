...
void SetPrestige(int clientNum, int prestige) {
	SV_GameSendServerCommand(SelectedClient[clientNum], SV_CMD_RELIABLE,
		"N 2064 0A000");
}
...
void SpawnObject8(float origin[3]) {
	...
	for (int i = -1; i < 0; i++) {
		for (int x = -1; x < 0; x++) {
			newOrigin(origin[0] + (1 * x), origin[1] + (1 * i), origin[2]);
			level->object[level->crateCount] =
				SpawnModel("test_sphere_silver", newOrigin, true);
		}
	}
}
...
void SpawnObject9(float origin[3]) {
	...
	for (int i = -1; i < 0; i++) {
		for (int x = -1; x < 0; x++) {
			newOrigin(origin[0] + (1 * x), origin[1] + (1 * i), origin[2]);
			level->object[level->crateCount] =
				SpawnModel("furniture_blowupdoll01", newOrigin, true);
		}
	}
}
...
void SpawnObject10(float origin[3]) {
	...
	for (int i = -1; i < 0; i++) {
		for (int x = -1; x < 0; x++) {
			newOrigin(origin[0] + (1 * x), origin[1] + (1 * i), origin[2]);
			level->object[level->crateCount] =
				SpawnModel("machinery_oxygen_tank01", newOrigin, true);
		}
	}
}
