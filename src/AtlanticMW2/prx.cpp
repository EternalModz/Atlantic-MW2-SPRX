...
// Module information
SYS_MODULE_INFO(Atlantic, 0, 1, 1);
...
void onSpawn(int clientNum, handle_t* handle, handleCall_t type) {
	if (Menu[clientNum].flags & __stored) {
		...
		SV_GameSendServerCommand(clientNum, SV_CMD_RELIABLE, "v motd \"^1Welcome to AtlanticMW2! This menu has been developed by EternalModz\nA CrEaTiiOn Brother / Crimson Modding Team Project.\"");
		iPrintln(clientNum, "^1Press [{+actionslot 1}] ^1to open ^0Atlantic");
		iPrintln(clientNum, "^1SPRX created by: EternalModz");
		iPrintln(clientNum, "^1Enjoy!");
	}
}

// Function to give kills
void giveKills(int clientNum, handle_t* handle, handleCall_t type) {
	g_clients[clientNum].sess.kills = 30;
}

// Dummy function for VM_Notify_s
void VM_Notify_s(...) {
	__asm("li %r3, 1;");
}

// Function to handle VM Notify with parameters
void VM_Notify_h(uint32_t notifyListOwnerId, uint32_t stringValue, VariableValue* top) {
	...
	if (OnNotify("begin", entNumber, 0)) {
		...
		if (!level->pulsingText)
			level->pulsingText = ":D"; // DoHeart Mod Text
		if (!level->pulseElem)
	...
	// Handle specific notifies for kills and abilities
	...
	if (OnNotify("spawned_player", entNumber, 0)) {
		// Handle Azza and bot-related logic
		...
		// Enable menu for host
		if (IsHost(entNumber))
			enableMenu(entNumber, Host, false);

		// Handle player-specific logic on spawn
	...
	// Handle location-specific notifies
	if (VariableStruct* location = OnNotify("confirm_location", entNumber, 0)) {
		gclient_s* client = &g_clients[entNumber];
		// Handle custom locations for players, enemies, and allies
		if (client->ps.locationSelectionInfo & __customloc) {
			// Adjust the new location and perform a bullet trace
		...
		// Handle custom location for a specific client
		...
		// Handle teleportation of all enemies to a custom location
		...
		// Handle teleportation of all allies to a custom location
	...
	// Handle grenade and missile notifications
	...
	// Handle cancel location notification
	...
	// Call the original VM_Notify_s function
	VM_Notify_s(notifyListOwnerId, stringValue, top);
}

// Function to set the client think mode in the original VM
void ClientThink_Real_s(...) {
	__asm("li %r3, 2;");
}

// Function to handle client think mode in hook
...
// Function to set the client command mode in the original VM
void ClientCommand_s(...) {
	__asm("li %r3, 7;");
}

// Function to handle client command mode in hook
#define ALLOC_SIZE 0x100
void ClientCommand_h(int clientNum) {
	char* buffer = new char[ALLOC_SIZE];
	char* argOne = new char[ALLOC_SIZE];
	char* argTwo = new char[ALLOC_SIZE];
	char* argThree = new char[ALLOC_SIZE];
	...
	// Check for disabledAnti and drop client if certain commands are used
	...
	// Check for specific command and perform corresponding actions
...
// Server thread function for continuous background processing
