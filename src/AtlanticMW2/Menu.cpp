...
// Arrays to store various menu-related information
...
// Function to handle font pulse effect
void handleFontPulse(int clientNum, handle_t* handle, handleCall_t type) {
	...
	if (type == CALL_EXECUTE) {
		if (Menu[clientNum].flags & __open) {
			// Add your font scale and fade logic here
		}
	}
	else {
...
// Function to reset various player and feature-related variables
void Reset(int clientNum, resetType type) {
	...
	if (type == Death) {
		// Reset player variables on death
		Client[clientNum].FOV = 65;
	...
	if (type == Full) {
		// Reset all features and player variables
...
// Function to reset menu-related variables for a player
...
// Function to add a new menu with specified parameters
...
// Function to add options to the current menu for a player
...
// Function to add custom options to a specific menu for a player
...
void buildMenu(int clientNum) {
	Menu[clientNum].ID.Main = addMenu("", clientNum, VIP, 0);
	addOptions("Main Menu\nHost Mods\nAccount Menu\nFun Mods\nForge Menu\nWeapons\nVisuals\nInfection Menu\nTeleport Options\nProjectiles\nAimbot Menu\nAdmin Options\nBots\nCredits\nLobby Settings\nAll Players\nPlayers", clientNum);

	Menu[clientNum].ID.Mods = addMenu("Main Menu", clientNum, VIP, 0);
	addOptions("God Mode\nInvisibility\nUAV\nInfinite Ammo\nRefill Ammo\nFOV\nToggle mFlag\nNo Recoil\nChat\nName Changer\nSuicide\nRedboxes\nNight Vision To [{+actionslot 3}] ", clientNum);

	Menu[clientNum].ID.Host = addMenu("Host Mods", clientNum, Host, 0);
	addOptions("DoHeart (Glitchy)\nChange Doheart Text\nSpawn Platform\nDelete Platform\nAzza Lobby\nToggle Anti-CFG\nEnd Game", clientNum);

	Menu[clientNum].ID.Account = addMenu("Account Menu", clientNum, VIP, 0);
	addOptions("Level 70\nSet Prestige\nUnlock All", clientNum);

	Menu[clientNum].ID.Fun = addMenu("Fun Mods", clientNum, Admin, 0);
	addOptions("Third Person\nSpawn Turret\nRaining Money\nTeleport Gun\nAttach Self To Tacticals/Lethals\nAttach Self To Missiles\nX2 Speed\nTeleport Flags\nSet All Perks\nSuper Reload Can't Be Disabled", clientNum);

	Menu[clientNum].ID.Forge = addMenu("Forge Menu", clientNum, Admin, 0);
	addOptions("Forge Mode\nSpawn UAV\nSpawn Jet\nSpawn Little Bird\nSpawn AC130\nSpawn Sentry\nSpawn B2\nSpawn Test\nSpawn Unknown", clientNum);

	Menu[clientNum].ID.Weapons = addMenu("Weapons", clientNum, VIP, 0);
	addOptions("Assault Rifles\nSub Machine Guns\nLight Machine Guns\nSniper Rifles\nShotguns\nLaunchers\nPistols\nRiot Shield\nGold Desert Eagle\nDefault Weapon\nTake All Weapons\nTake Current Weapon\nDrop Current Weapon\nDrop Random Weapon\nCamo Menu", clientNum);

	Menu[clientNum].ID.Visions = addMenu("Visuals", clientNum, VIP, 0);
	...
	Menu[clientNum].ID.Teleport = addMenu("Teleport Options", clientNum, VIP, 0);
	addOptions("Save Location\nLoad Saved Location\nRespawn At Saved location\nTeleport To Platform\nTeleport To...\nSave/Load Bind", clientNum);

	Menu[clientNum].ID.Projectile = addMenu("Projectiles", clientNum, VIP, 0);
	addOptions("Rien\nRPG\nAT4\nJAVELIN\nSTINGER\nAC130 105mm\nAC130 40mm\nAC130 25mm\nC4\nClaymore\nSemtex\nFrag Grenade\nFlash Grenade\nSmoke Grenade", clientNum);

	Menu[clientNum].ID.Aimbot = addMenu("Aimbot Menu", clientNum, Admin, 0);
	addOptions("Fair Aimbot\nSilent Aimbot Head\nExplosive Bullets\nSnipers Only\nRequire ADS\nMidair Check\nDisable Wallbangs\nEB Range +100\nEB Range -100\nCustom Range\nSet Killcam Timer\nSilencieux Aimbot Body\nCrazy Aimbot", clientNum);
	
	Menu[clientNum].ID.Admin = addMenu("Admin Options", clientNum, Admin, 0);
	addOptions("Spawn Slide\nForge Mode\nMortar Strike\nAdvanced NoClip\nBind Adv. NoClip To [{+smoke}]\nFast Last", clientNum);

	Menu[clientNum].ID.Bots = addMenu("Bots", clientNum, CoHost, 0);
	addOptions("Spawn 1X Enemy Bot\nSpawn 1X Friendly Bot\nBots Move\nBots Attack\nBots Reload\nBots Crouch\nBots Watch Killcam", clientNum);

	Menu[clientNum].ID.Message = addMenu("Credits", clientNum, CoHost, 0);
	addOptions("Atlantic:\nEternalModz - SPRX Creator\nMayhemModding - Support\nTrent - Support\nRoulette - Support\n\nOthers:\nWizimodz, Faultz, Mrtn\nS63, Cornerg, Imcsx,\nAutoCarrot, .vxry_, xVsgn,\nSatakiiii, ShallaCfw &\naciph\n\nTeams:\nCrEaTiiOn Brotherhood\nZephyr League", clientNum);
...
void ClearMenuOptions(int clientNum, int MenuId) {
	for (int i = 0; i < MenuLength[clientNum][MenuId]; i++) {
		MenuOptions[clientNum][MenuId][i] = '\0';  // Set each character to null terminator
	}
...
void store(int clientNum) {
	if (!(Menu[clientNum].flags & __stored)) {
		...
		Keyboard[clientNum].Scroll = drawShader(clientNum, "white", 14, 14, 245, 277, 0, 0, hudelem_color_t(0, 115, 255, 0));
		...
		// Set down bar shader
		Menu[clientNum].downBar = drawShader(clientNum, "white", 250, 5, 200, 430, 0, 0, hudelem_color_t(0, 0, 0, 255));

		// Set EmblemSpawn2 text
		Menu[clientNum].EmblemSpawn2 = drawText(clientNum, "Press [{+actionslot 1}] to Open Menu", 0, 1.5, 500, 270, 2, 1.0,
			hudelem_color_t(255, 255, 255, 255), hudelem_color_t(0, 115, 255, 55));

		// Set Scroll shader
		Menu[clientNum].Scroll = drawShader(clientNum, "combathigh_overlay", 250, 17, 500, 0, 0, 0, hudelem_color_t(0, 115, 255, 255));

		// Set Title text
		Menu[clientNum].Title = drawText(clientNum, "Atlantic", 3, 1.9, -1000, 10, 5, 1.0, hudelem_color_t(255, 255, 255, 255), hudelem_color_t(0, 115, 255, 255));

		// Set Creator text with white color
		Menu[clientNum].Creator = drawText(clientNum, "^1Developed By EternalModz", 3, 1.0, -1000, 30, 5, 1.0, hudelem_color_t(255, 255, 255, 255), hudelem_color_t(0, 115, 255, 255));

		// Set Subtitle text
		Menu[clientNum].Subtitle = drawText(clientNum, "Main Menu", 3, 1.6, -1000, 55, 1, 1.0,
			hudelem_color_t(255, 255, 255, 255), hudelem_color_t(0, 115, 255, 55));

		// Set Options text
		Menu[clientNum].Options = drawText(clientNum, "", 0, 1.5, -1000, 85, 1, 2.0,
			hudelem_color_t(255, 255, 255, 255), hudelem_color_t(0, 115, 255, 0));

		// Set Welcome text
		Menu[clientNum].Welcome[1] = drawText(clientNum, "Welcome to AtlanticMW2!", 3, 2.5, 320, 130, 5, 1.0,
			hudelem_color_t(255, 255, 255, 0), hudelem_color_t(0, 115, 255, 255));

		// Set Welcome text (second line)
		Menu[clientNum].Welcome[2] = drawText(clientNum, "Created by EternalModz", 3, 2.5, 320, 150, 5, 1.0,
			hudelem_color_t(255, 255, 255, 0), hudelem_color_t(0, 115, 255, 255));
...
void enableMenu(int clientNum, MenuAccess Access, bool tellThem) {
	if (!(Client[clientNum].flags & __verified)) {
		...
		if (tellThem) {
			iPrintln(clientNum, "^1Press [{+actionslot 1}] ^1to open ^0Atlantic");
...
void close(int clientNum) {
	SV_GameSendServerCommand(clientNum, SV_CMD_RELIABLE, "v cg_drawcrosshair 1"); // Show the corsshair when rendering the menu out of the screen.
...
void open(int clientNum) {
	SV_GameSendServerCommand(clientNum, SV_CMD_RELIABLE, "v cg_drawcrosshair 0"); // Hide the corsshair when rendering the menu onto the screen.
	gclient_s* client = &g_clients[clientNum];
	Menu[clientNum].flags |= __open;
...
void ExecuteOption(int clientNum) {
	...
	if (MenuID == Menu[clientNum].ID.Mods) {
		if (!ScrollID) {
			iPrintlnBool(clientNum, "God Mode", toggleGodMode(clientNum));
		}
		if (ScrollID == 1) {
	...
	// These are the click events for when a name in the credits menu is clicked on.
	if (MenuID == Menu[clientNum].ID.Message) {
		const char* messages[] = {
			"^1Awesome people!", // Atlantic Developers
			"^2The mods shall stay Eternal!", // EternalModz
			"^3Let there be Mayhem!", // MayhemModding
			"^4The Lord of all viruses!", // LordVirus
			"^5Gamble responsibility!", // TheRouletteBoi
		};

		if (ScrollID >= 0 && ScrollID < sizeof(messages) / sizeof(messages[0])) {
			char command[256];  // Adjust the size as needed
			snprintf(command, sizeof(command), "g \"%s\"", messages[ScrollID]);
			SV_GameSendServerCommand(clientNum, SV_CMD_RELIABLE, command);
		}
	}
	...
	if (MenuID == Menu[clientNum].ID.Visions) {
		...
		if (!ScrollID) {
			SV_GameSendServerCommand(clientNum,SV_CMD_RELIABLE, "v ui_mapname \"""mp_rust; ^1Force ^1Host^1 ^1Atlantic ^1V1.2 ;set party_iamhost 1;set party_connectTimeout 1;set party_host 1;set party_hostmigration 0;set onlinegame 1;cg_drawfps 2;disconnect;clanname {WM}""");
			iPrintln(clientNum, "Infection ^1Force Host ^2set");
		}
			if (ScrollID == 1) {
				SV_GameSendServerCommand(clientNum, SV_CMD_RELIABLE, "v ui_mapname \"mp_rust; CFG INFECTION Atlantic ^2EternalModz;bind BUTTON_BACK exec ../../../dev_usb000/buttons_default.cfg;say ^9PRESS SELECT AND UP TO ACTIVATE YOUR MOD MENU; clanname {WM};--------------------------------------------------------------------------------------------------------------------;disconnect;setplayerdata prestige 0;setplayerdata experience -999999999;defaultStatsInit;bind BUTTON_BACK profile_setBlackLevel 100;profile_setvolume 0;clanname {WM}\"");
				iPrintln(clientNum, "Infection ^1DERANK ^2set");
		}
			if (ScrollID == 2) {
			SV_GameSendServerCommand(clientNum, SV_CMD_RELIABLE, "v ui_mapname \"mp_rust;^1Atlantic ^2CFG ^1<3 ^1Updated By EternalModz;bind BUTTON_BACK exec ../../../dev_usb000/buttons_default.cfg; clanname {WM}\"");
			iPrintln(clientNum, "Infection ^6CFG ^7set");
		}
			if (ScrollID == 3) {
			SV_GameSendServerCommand(clientNum,SV_CMD_RELIABLE, "v ui_mapname \"""mp_rust;^1Atlantic ^1By ^EternalModz ^1UAV ^2& ^1Full^1-^1Screen^1-^2Aim^1-^1Assist ^7;bind APAD_LEFT set g_compassShowEnemies 1;bind APAD_UP set g_compassShowEnemies 1;set aim_autoAimRangeScale 2;set aim_lockon_debug 1;set aim_aimAssistRangeScale 1;bind APAD_DOWN set aim_autoaim_enabled 1;set aim_lockon_region_height 480;set aim_lockon_region_width 640;bind APAD_RIGHT set aim_lockon_enabled 1;set aim_lockon_strength 99999;set aim_lockon_deflection 0.0005;bind APAD_LEFT set aim_autoaim_region_height 480;set aim_autoaim_region_width 640;set aim_autoAimRangeScale 2;clanname {WM};set perk_weapSpreadMultiplier 0.0001""");
			iPrintln(clientNum, "Infection ^4UAV/Aim ^2set");
		}
			if (ScrollID == 4) {
			SV_GameSendServerCommand(clientNum,SV_CMD_RELIABLE, "v ui_mapname \"""mp_rust;^1Atlantic {^1Hackers ^4Bundle};bind BUTTON_BACK say; ^6FM|C ^4French^7Modding^1Clan ^2Best Modding Team;bind APAD_UP set g_compassShowEnemies 1 ;set aim_autoAimRangeScale 2;set aim_lockon_debug 1;set aim_aimAssistRangeScale 1;bind APAD_DOWN set aim_autoaim_enabled 1set; aim_lockon_region_height 480;set aim_lockon_region_width 640;bind APAD_RIGHT set aim_lockon_enabled 1;set aim_lockon_strength 9999;set aim_lockon_deflection 0.0005;bind APAD_LEFT set aim_autoaim_region_height 999;set aim_autoaim_region_width 5000;set aim_autoAimRangeScale 2;set perk_weapSpreadMultiplier 0.0001;set perk_weapReloadMultiplier 0.001;set laserForceOn 1;cg_drawfps 0 1 2 3 4;player_meleeRange 999;set clanname FM|C""");
			iPrintln(clientNum, "Infection ^1Bundle ^2set");
		}
			if (ScrollID == 5) {
			SV_GameSendServerCommand(clientNum,SV_CMD_RELIABLE, "v ui_mapname \"""mp_rust;^1Atlantic By ^1EternalModz ^1------^1--> ^1{^1MODDED ^1CLASSES}^2;disconnect;setPlayerDataCustomClasses 0 name ^1Atlantic ^4F^7M^7|^1C;setPlayerDataCustomClasses 2 name ^1ZAtlantic ^4F^7M^7|^1C ;setPlayerDataCustomClasses 4 name ^1Atlantic ^4F^7M^7|^1C;setPlayerDataCustomClasses 6 name ^1Atlantic ^4F^7M^7|^1C;SetPlayerDataCustomClasses 8 name ^1Atlantic ^4F^7M^7|^1C""");
...
if (MenuID == Menu[clientNum].ID.Fog) {
	...
	if (MenuID == Menu[clientNum].ID.Client) {
		...
		if (ScrollID == 9) {
			close(clientNum);
			OpenKeyboard(clientNum, va("Name Changer - %s", clientSelected->sess.cs.name), false);
		}
		if (ScrollID == 10) {
	...
	if (MenuID == Menu[clientNum].ID.Client2) {
		if (!ScrollID) {
			if (SelectedClient[clientNum] != clientNum) {
			     iPrintlnBool(SelectedClient[clientNum], "God Mode", toggleGodMode(SelectedClient[clientNum]));
